...
**Local-only directories** (not versioned, ignored by `.gitignore`):
- `.local/` - Patches, scripts, and documentation for local development
  - `..local/patches/` - Patch files for downstream distributions
  - `.local/docs/` - Internal documentation
  - `.local/scripts/` - Development utilities
- `.claude/` - Claude Code context and project notes

**Repository structure**:
- `master` - Pristine copy of Piwigo (synced with upstream)
- `feature/*` - Feature development branches
- `patches/X.X.X` - Release-specific patches and adaptations
...
## Step 9️⃣ bis: Add Patch Header Metadata

Edit the newly generated patch file to add header metadata **at the very beginning** (before the first `diff`):

```bash
# Edit your patch file to add metadata
...
```
# Issue: 2534
# Title: Admin Menubar Hook
# Description: Adds admin_menubar_plugin_links hook for plugins
# Dependencies: 2533
# Author: Gotcha
# Date: 2026-04-04
# Status: tested
...
# Title: Admin Menubar Hook
# CPV – Compatible Piwigo Version: 16.3.0
# Description: Adds admin_menubar_plugin_links hook for plugins to register menubar items
# Dependencies: 2533
# Author: Gotcha
# Date: 2026-04-04
# Status: tested
...
**Important:**
- The header must come **before** the first `diff --git` line
- Separate metadata from the diff with a **blank line** (recommended but not required)
- Only **CPV is mandatory**; all other fields are optional
- CA-Patcher will warn if CPV is missing or invalid
...
These files are **NOT versioned** (ignored by `.gitignore`) to keep the repository clean:

- `.local/` - All patch files, scripts, and documentation
- `.claude/` - Claude Code context and AI assistant notes

**Why?** Keeping the repository clean ensures:
- PRs to upstream Piwigo contain only code changes
- No pollution with local-specific files
- Clear separation between code and utilities

**Access**: These directories persist across branch checkouts since Git ignores them.
